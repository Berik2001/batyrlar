/**
 * i18n: словарь интерфейса + переключатель языка.
 * Статичные строки размечены data-i18n="key" (textContent) или
 * data-i18n-attr="attr:key;attr2:key2". Динамические блоки слушают событие "langchange".
 */

export const LANGS = ["kk", "ru", "en"];
const STORAGE_KEY = "batyrlar.lang";
...
function resolveInitialLang() {
  ...
  } catch { /* storage недоступен — используем дефолт */ }
  return "kk";
}
...
export const t = (key, vars) => {
  const str = DICT[current][key] ?? DICT.kk[key] ?? key;
  return vars ? str.replace(/\{(\w+)\}/g, (_, k) => vars[k] ?? "") : str;
};

/** Выбор перевода из объекта данных {kk, ru, en}. */
export const pick = (value) => (value && typeof value === "object" ? value[current] ?? value.kk : value ?? "");
