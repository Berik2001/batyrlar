...
Сайт живёт на GitHub Pages и пересобирается сам при каждом пуше в `main` —
workflow [`.github/workflows/pages.yml`](.github/workflows/pages.yml).

Next-версия собирается в статику (`output: "export"`): сервера нет, все 17 страниц
готовятся заранее. Репозиторий не корневой, поэтому сайт отдаётся из подпапки —
при сборке задаётся `NEXT_PUBLIC_BASE_PATH=/batyrlar`, и пути к файлам из `public`
проходят через `asset()` (`web/src/lib/asset.ts`), иначе Next префикс к ним не добавит.
Локально переменная пустая, и разработка идёт по обычным адресам.

Первая, статическая версия кладётся рядом, в `/classic/`.

Ключ `YOUTUBE_API_KEY` не обязателен: без него список роликов берётся из репозитория.
Если он понадобится, добавьте его в Settings → Secrets and variables → Actions.
